...
// Main function
int true_electron_rate_2D(const char* input, const char* output, int pdg_number) {
    ...
    std::map<std::string,TH2D*> Histo_Map_total_pt;
    Histo_Map_total_pt["h_pass_total_pt"] = new TH2D("h_pass_total_pt", ";#theta [mrad]", 30, 0, TMath::Pi()*1000, 30, 0, 100);  
    Histo_Map_total_pt["h_electron_total_pt"] = new TH2D("h_electron_total_pt", ";#theta [mrad]", 30, 0, TMath::Pi()*1000, 30, 0, 100);
    Histo_Map_total_pt["h_pion_total_pt"] = new TH2D("h_pion_total_pt", ";#theta [mrad]", 30, 0, TMath::Pi()*1000, 30, 0, 100);


    std::map<std::string,TH2D*> Histo_Map_total_p;
    Histo_Map_total_p["h_pass_total_p"] = new TH2D("h_pass_total_p", ";#theta [mrad]", 30, 0, TMath::Pi()*1000, 30, 0, 100);   
    Histo_Map_total_p["h_electron_total_p"] = new TH2D("h_electron_total_p",";#theta [mrad]", 30, 0, TMath::Pi()*1000, 30, 0, 100);
    Histo_Map_total_p["h_pion_total_p"] = new TH2D("h_pion_total_p", ";#theta [mrad]", 30, 0, TMath::Pi()*1000, 30, 0, 100);
        ...
        for (const auto& mcp: mcparticles) {
            ...
            if (particle.flag == true) {
                ...
                if(reco_p.reco.first.getPDG() == mcp.getPDG() && reco_p.trwgt.first > 0.75) {
                    Histo_Map_pt["h_pass_pt"]->Fill(particle.theta*1000,particle.pt,reco_p.trwgt.first);
                    Histo_Map_p["h_pass_p"]->Fill(particle.theta*1000,particle.p, reco_p.trwgt.first);
                    Histo_Map_total_pt["h_pass_total_pt"]->Fill(particle.theta*1000,particle.pt);
                    Histo_Map_total_p["h_pass_total_p"]->Fill(particle.theta*1000,particle.p);
                }
                else {                                     
                    if (std::abs(reco_p.reco.first.getPDG()) == 11) {
                        Histo_Map_pt["h_electron_pt"]->Fill(particle.theta*1000, particle.pt, reco_p.trwgt.first);
                        Histo_Map_total_pt["h_electron_total_pt"]->Fill(particle.theta*1000, particle.pt);
                    }
                    else if (std::abs(reco_p.reco.first.getPDG()) == 211){
                        Histo_Map_pt["h_pion_pt"]->Fill(particle.theta*1000,particle.pt, reco_p.trwgt.first);
                        Histo_Map_total_pt["h_pion_total_pt"]->Fill(particle.theta*1000,particle.pt);                      
                    }

                    if (std::abs(reco_p.reco.first.getPDG()) == 11) {
                        Histo_Map_p["h_electron_p"]->Fill(particle.theta*1000, particle.p, reco_p.trwgt.first);
                        Histo_Map_total_p["h_electron_total_p"]->Fill(particle.theta*1000, particle.p);
                    }
                    else if (std::abs(reco_p.reco.first.getPDG()) == 211){
                        Histo_Map_p["h_pion_p"]->Fill(particle.theta*1000,particle.p, reco_p.trwgt.first);
                        Histo_Map_total_p["h_pion_total_p"]->Fill(particle.theta*1000,particle.p);                      
                    }
                }               
            }
        }
    ...
    Histo_Map_pt["h_pass_pt"]->Divide(Histo_Map_total_pt["h_pass_total_pt"]);
    Histo_Map_pt["h_electron_pt"]->Divide(Histo_Map_total_pt["h_electron_total_pt"]);
    Histo_Map_pt["h_pion_pt"]->Divide(Histo_Map_total_pt["h_pion_total_pt"]);

    Histo_Map_p["h_pass_p"]->Divide(Histo_Map_total_p["h_pass_total_p"]);
    Histo_Map_p["h_electron_p"]->Divide(Histo_Map_total_p["h_electron_total_p"]);
    Histo_Map_p["h_pion_p"]->Divide(Histo_Map_total_p["h_pion_total_p"]);
    ...
    for (auto& histo: Histo_Map_pt) { 
        ...
        c_other_pt->Draw();
        c_other_pt->SaveAs(Form("plots/True_electron_rate/%s_thetapt_trwgt_weighted.pdf", misname.c_str()));
    }
    for (auto& histo: Histo_Map_p) { 
        ...
        t.DrawLatexNDC(0.15, 0.02, Form("#scale[0.8]{%s}", misname.c_str()));
        c_other_p->Draw();
        c_other_p->SaveAs(Form("plots/True_electron_rate/%s_thetap_trwgt_weighted.pdf", misname.c_str()));
    } 
